...
int main(int argc, char *argv[]) {
    ...
    if (argc < 5) {
        printf("Usage: %s <L> <lambda> <start_index> <end_index>\n", argv[0]);
        printf("  L: lattice size\n");
        printf("  lambda: spontaneous red-to-blue conversion rate\n");
        ...
        printf("  Job 0: %s 50 0.10 0 10\n", argv[0]);
        printf("  Job 1: %s 50 0.10 10 20\n", argv[0]);
        printf("  Job 999: %s 50 0.10 9990 10000\n", argv[0]);
        return 1;
    }
    
    // Parse command line arguments
    int L = atoi(argv[1]);
    double lambda_rate = atof(argv[2]);
    ...
    if (lambda_rate < 0) {
        printf("Error: lambda must be non-negative\n");
        return 1;
    }
    ...
    // Create filename based on L, lambda, and indices
    char filename[150];
    sprintf(filename, "escape_prob_L%d_lambda%.2f_%d_%d.txt", L, lambda_rate, start_index, end_index);
    ...
    fprintf(file, "# p\tescape_probability\n");
    fprintf(file, "# Realizations: %d (indices %d to %d)\n", num_realizations, start_index, end_index - 1);
    
    printf("Running simulations for L = %d, lambda = %.2f\n", L, lambda_rate);
    printf("Realizations: %d (indices %d to %d)\n", num_realizations, start_index, end_index - 1);
    printf("Saving results to %s\n", filename);
    printf("Progress: ");
    
    // Run simulations for 11 values of p from 1.95 to 2.0
    for (int i = 0; i < num_p_values; i++) {
        double p = 1.95 + (0.05 * i) / (num_p_values - 1.0);
        ...
        // Print progress
        printf("%.1f%% ", 100.0 * (i + 1) / num_p_values);
        fflush(stdout);
    }
